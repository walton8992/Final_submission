...
class one_Model:
            ...
            results = list(
                tqdm(
                    executor.map(
                        models.model,
                        itertools.repeat(
                            search_algo, len(self.tuple_arguments)
                        ),
                        itertools.repeat(model, len(self.tuple_arguments)),
                        self.tuple_arguments,
                        itertools.repeat(
                            sensitivity_threshold, len(self.tuple_arguments)
                        ),
                        itertools.repeat(
                            self.dict_sites_melt, len(self.tuple_arguments)
                        ),
                        itertools.repeat(min_step, len(self.tuple_arguments)),
                        itertools.repeat(
                            penalty_val, len(self.tuple_arguments)
                        ),
                        itertools.repeat(
                            hour_limit, len(self.tuple_arguments)
                        ),
                    ),
    ...
    def generate_models(self):
        ...
        model7 = self.thread_Process_model(
            "Window", 1, "rbf", 100, penalty_val=100
        )
