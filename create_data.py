...
def create_dictionary(melt: bool, table: pd.DataFrame) -> dict:
    ...
    for name in list_sites:
        ...
        dict_test[name].loc[:, "datetime"] = pd.to_datetime(
            dict_test[name].datetime
        )
        if melt:
            dict_test[name] = dict_test[name].melt(
                id_vars=["name", "datetime"]
            )
    return dict_test
...
def create_data(file_location_database, file_location_sites):
    path = file_location_database + "ping_practicum.db"
